from os import system, listdir, makedirs, remove, path
...
def create_raster_tiles(region, tiff_file, year, source = GEOTIFF, dest = RASTER_TILES, zoom = 8):
    ...
    if not path.isdir(dest_dir + year):
        makedirs(dest_dir + year)
    else:
        print("Did Not Create Raster Tiles for year %d since directory already exists")
        return
